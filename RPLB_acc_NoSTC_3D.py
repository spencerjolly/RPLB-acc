...
@jit(nopython=True)
def RPLB_acc_NoSTC_3D(lambda_0, tau_0, w_0, P, Psi_0, phi_2, t_0, z_0, x_0, y_0, beta_0):
    ...
    for k in range(0, len(time)-1):
        ...
        B_x_time = pulse_prep*((c_2)*eps +
                               ((1/2)*c_3 + (1/2)*c_4*rho**2 - (1/4)*c_5*rho**4)*eps**3 +
                               ((3/8)*c_4 + (3/8)*c_5*rho**2 + (3/16)*c_6*rho**4 -
                                (1/4)*c_7*rho**6 + (1/32)*c_8*rho**8)*eps**5)*np.exp(+1j*np.pi/2)*(-y[k]/w_0)/c
        
        B_y_time = pulse_prep*((c_2)*eps +
                               ((1/2)*c_3 + (1/2)*c_4*rho**2 - (1/4)*c_5*rho**4)*eps**3 +
                               ((3/8)*c_4 + (3/8)*c_5*rho**2 + (3/16)*c_6*rho**4 -
                                (1/4)*c_7*rho**6 + (1/32)*c_8*rho**8)*eps**5)*np.exp(+1j*np.pi/2)*(x[k]/w_0)/c

        E_z_total = np.real(Amp*trans*E_z_time)
        E_x_total = np.real(Amp*trans*E_x_time)
        E_y_total = np.real(Amp*trans*E_y_time)
        dot_product = v_z[k]*E_z_total + v_x[k]*E_x_total + v_y[k]*E_y_total
        B_x_total = np.real(Amp*trans*B_x_time)
        B_y_total = np.real(Amp*trans*B_y_time)

        deriv2[k] = (-q_e/(gamma[k]*m_e))*(E_z_total+v_x[k]*B_y_total-v_y[k]*B_x_total-v_z[k]*dot_product/(c**2))  # Force in z
        deriv4[k] = (-q_e/(gamma[k]*m_e))*(E_x_total-v_z[k]*B_y_total-v_x[k]*dot_product/(c**2))  # Force in x
        deriv6[k] = (-q_e/(gamma[k]*m_e))*(E_y_total+v_z[k]*B_x_total-v_y[k]*dot_product/(c**2))  # Force in y
