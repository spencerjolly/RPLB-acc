...
def RPLB_acc_NoSTC(lambda_0, tau_0, w_0, P, Psi_0, phi_2, t_0, z_0, beta_0):
    ...
    w_0 = beam waist at the focal plane [m]
    ...
    n = (lambda_0/(0.8e-6))*np.maximum(50, np.round(np.sqrt(P*tau_0/(tau*w_0**2))/(3e10)))  # empirically chosen resolution based on field strength
    ...
    for k in range(0, len(time)-1):
        ...
        field_temp = np.exp(1j*(Psi_0+2*np.arctan(z[k]/z_R)+omega_0*time[k]-omega_0*z[k]/c))/(z_R*(1+(z[k]/z_R)**2))
